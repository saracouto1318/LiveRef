    ...
    public HashSet<Cluster> getClusters(ArrayList<PsiElement> entities) {
        ...
        while (clusters.size() > 2) {
            ...
            if (minRow < minCol) {
                ...
                distanceMatrix = UtilitiesOverall.deleteRows(distanceMatrix, minRow, minCol);
                distanceMatrix = UtilitiesOverall.deleteColumns(distanceMatrix, minCol);
                distanceMatrix = UtilitiesOverall.insertRows(distanceMatrix, minRow, newDistances);
                distanceMatrix = UtilitiesOverall.deleteColumns(distanceMatrix, minRow);
                distanceMatrix = UtilitiesOverall.insertColumns(distanceMatrix, minRow, newDistances);
                ...
                distanceMatrix = UtilitiesOverall.deleteRows(distanceMatrix, minRow, minCol);
                distanceMatrix = UtilitiesOverall.deleteColumns(distanceMatrix, minRow);
                distanceMatrix = UtilitiesOverall.insertRows(distanceMatrix, minCol, newDistances);
                distanceMatrix = UtilitiesOverall.deleteColumns(distanceMatrix, minCol);
                distanceMatrix = UtilitiesOverall.insertColumns(distanceMatrix, minCol, newDistances);
            ...
            for (Cluster cluster : clusters) {
                if(cluster.getEntities().size() >= ThresholdsCandidates.minNumStatements &&
                        (refactorUtils.getAllStatements(candidates.get(0).method).size() - cluster.getEntities().size()) <=
                                (ThresholdsCandidates.minOrigMethodPercentageEC * refactorUtils.getAllStatements(candidates.get(0).method).size())){
