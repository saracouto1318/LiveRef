package liverefactoring.ui.JDeodorant;
...
import liverefactoring.utils.JDeodorant.RefactoringUtility;
import liverefactoring.utils.JDeodorant.math.AdjacencyList;
import liverefactoring.utils.JDeodorant.math.Edge;
import liverefactoring.utils.JDeodorant.math.Node;
import liverefactoring.utils.JDeodorant.math.TarjanAlgorithm;
import liverefactoring.utils.JDeodorant.utils.ExpressionExtractor;
import liverefactoring.utils.JDeodorant.utils.MethodDeclarationUtility;
import liverefactoring.utils.JDeodorant.utils.StatementExtractor;
...
public class ExtractClassRefactoring {
    ...
    private static final String UPDATED_EXTRACTED_FIELD_ASSIGNMENT = IntelliJDeodorantBundle.message("god.class.preview.updated.extracted.field.assignment");
    ...
    private final Map<PsiMethod, PsiMethod> updatedSourceMethodToInitialCopy = new LinkedHashMap<>();
    /*
    ...
    private final ExtractClassPreviewProcessor previewProcessor = new ExtractClassPreviewProcessor();
    private final List<ExtractClassPreviewProcessor.PsiMethodComparingPair> methodComparingList = previewProcessor.getMethodComparingList();
    private final Map<PsiMethod, ArrayList<ExtractClassPreviewProcessor.PsiElementComparingPair>> methodElementsComparingMap = previewProcessor.getMethodElementsComparingMap();
    /**
    ...
    public void setPreviewUsage() {
        ...
        ExtractClassPreviewProcessor.CopiedData clone = previewProcessor.cloneSourceFile(sourceFile, sourceTypeDeclaration, extractedFieldFragments, extractedMethods, delegateMethods);
        this.sourceFile = clone.getFileToUpdate();
        this.sourceTypeDeclaration = clone.getClassToUpdate();

        init(extractedFieldFragments, extractedMethods, delegateMethods, extractedTypeName);

        updatedSourceElementsToInitial = previewProcessor.getUpdatedSourceElementsToInitial();
        previewProcessor.setInitialSourceClass(new ExtractClassPreviewProcessor.PsiClassWrapper(clone.getSourceClassInitialCopy(), UPDATED_SOURCE_CLASS));
        previewProcessor.setUpdatedSourceClass(new ExtractClassPreviewProcessor.PsiClassWrapper(sourceTypeDeclaration, ""));
    }
    ...
    public ExtractClassPreviewProcessor getPreviewProcessor() {
                ...
                if (psiMethod.isConstructor()) {
                    ...
                    if (previewUsage) {
                        previewProcessor.addToPsiElementChangesFromUpdatedClass(
                                new ExtractClassPreviewProcessor.PsiElementChange(
                                        psiStatement,
                                        ExtractClassPreviewProcessor.PsiElementChange.ChangeType.ADD_BEFORE,
    ...
    private void addDelegationInExtractedMethod(PsiMethod sourceMethod) {
        ...
        if (previewUsage) {
            ...
            previewProcessor.getMethodComparingList().add(new ExtractClassPreviewProcessor.PsiMethodComparingPair(
                    initialSourceMethod,
                    sourceMethod,
    ...
    private void removeSourceMethods(Set<PsiMethod> methods) {
        ...
        for (PsiMethod method : methods) {
            ...
            if (previewUsage) {
                previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(
                        method,
                        ExtractClassPreviewProcessor.PsiElementChange.ChangeType.REMOVE,
                            ...
                            if (expression instanceof PsiMethodCallExpression) {
                                ...
                                if (extractedMethods.contains(method)) {
                                    ...
                                    if (qualifier == null || qualifier instanceof PsiThisExpression) {
                                        ...
                                        if (additionalArgumentsAddedToMovedMethod != null) {
                                            ...
                                            for (PlainVariable argument : additionalArgumentsAddedToMovedMethod) {
                                                ...
                                                if (isThisVariable(argument)) {
                                                    ...
                                                    if (argument.isField()) {
                                                        if (index != (additionalArgumentsAddedToMovedMethod.size() - 1))
                                                            arguments.append("this.").append(argument.getName()).append(", ");
                                                        else
                                                            arguments.append("this.").append(argument.getName());
                                                    } else {
                                                        if (index != (additionalArgumentsAddedToMovedMethod.size() - 1))
                                                            arguments.append(argument.getName()).append(", ");
                                                        else
                                        ...
                                        arguments = new StringBuilder(arguments.toString().replaceAll(", , ", ", "));
    ...
    private PsiJavaFile createExtractedClass(Set<PsiField> modifiedFieldsInNonExtractedMethods, Set<PsiField> accessedFieldsInNonExtractedMethods) {
        ...
        previewProcessor.setExtractedClass(new ExtractClassPreviewProcessor.PsiClassWrapper(extractedClass, CREATED_EXTRACTED_CLASS));
    ...
    private void createExtractedMethodDeclaration(PsiMethod extractedMethod, PsiClass extractedClass) {
        ...
        for (PsiAnnotation annotation : extractedMethod.getAnnotations()) {
            if (annotation.getQualifiedName() != null && annotation.getQualifiedName().equals("Test")) {
                annotation.delete();
            }
        }
    ...
    private void updateBodyDeclarationAccessModifier(PsiClass memberType) {
        ...
        if (previewUsage) {
            previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(memberType.getModifierList(), ExtractClassPreviewProcessor.PsiElementChange.ChangeType.REPLACE, CHANGED_ACCESS_TO_PUBLIC));
        }
    }
    ...
    private void setPublicModifierToSourceField(PsiField variableBinding) {
        ...
        if (previewUsage) {
            previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(variableBinding, ExtractClassPreviewProcessor.PsiElementChange.ChangeType.REPLACE, CHANGED_ACCESS_TO_PUBLIC));
        }
    }
    ...
    private void createExtractedTypeFieldReferenceInSourceClass(ModifyType modifyType) {
            ...
            for (PsiMethod constructor : constructorFinalFieldAssignmentMap.keySet()) {
                ...
                if (modifyType == ModifyType.SOURCE) {
                    ...
                    if (!insertAfterStatements.isEmpty()) {
                        ...
                        if (previewUsage) {
                            previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(
                                    assignmentStatement.copy(),
                                    ExtractClassPreviewProcessor.PsiElementChange.ChangeType.ADD_AFTER,
        ...
        if (modifyType == ModifyType.SOURCE) {
            ...
            if (previewUsage) {
                previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(extractedReferenceFieldDeclaration, ExtractClassPreviewProcessor.PsiElementChange.ChangeType.ADD_AFTER, CREATED_FIELD_HOLDING_REFERENCE, sourceTypeDeclaration.getLBrace()));
            }
        }
    }
    ...
    private void updateWriteObjectInSourceClass(String modifiedExtractedTypeName) {
            ...
            if (isWriteObject(method)) {
                ...
                if (!extractedMethods.contains(method)) {
                    ...
                    if (previewUsage) {
                        ...
                        previewProcessor.getMethodComparingList().add(new ExtractClassPreviewProcessor.PsiMethodComparingPair(initialMethod, method, UPDATE_WRITE_OBJECT, true));
                    }
                }
            }
        }
        if (!methodFound) {
            ...
            if (previewUsage) {
                previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(writeObjectMethod, ExtractClassPreviewProcessor.PsiElementChange.ChangeType.ADD_BEFORE, CREATE_WRITE_OBJECT, sourceTypeDeclaration.getRBrace()));
            }
        }
    }
    ...
    private void updateReadObjectInSourceClass(String modifiedExtractedTypeName) {
        ...
        for (PsiMethod method : methodDeclarations) {
            if (isReadObject(method)) {
                ...
                if (!extractedMethods.contains(method)) {
                    ...
                    if (previewUsage) {
                        PsiMethod initialMethod = (PsiMethod) updatedSourceElementsToInitial.get(method);
                        previewProcessor.getMethodComparingList().add(new ExtractClassPreviewProcessor.PsiMethodComparingPair(initialMethod, method, UPDATE_READ_OBJECT, true));
                    }
                }
            }
        }
        ...
        if (!methodFound) {
            ...
            if (previewUsage) {
                previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(readObjectMethod, ExtractClassPreviewProcessor.PsiElementChange.ChangeType.ADD_AFTER, CREATE_READ_OBJECT, sourceTypeDeclaration.getLBrace()));
            }
        }
    }
    ...
    private void updateCloneInSourceClass(String modifiedExtractedTypeName) {
                ...
                if (!extractedMethods.contains(method)) {
                    ...
                    if (methodBody != null) {
                        ...
                        if (firstStatement != null) {
                            ...
                            if (previewUsage) {
                                PsiMethod initialMethod = (PsiMethod) updatedSourceElementsToInitial.get(method);
                                previewProcessor.getMethodComparingList().add(new ExtractClassPreviewProcessor.PsiMethodComparingPair(initialMethod, method, UPDATE_CLONE, true));
                            }
                        }
                    }
                }
        ...
        if (!methodFound) {
            ...
            if (cloneMethodBinding != null) {
                ...
                if (previewUsage) {
                    previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(cloneMethodDeclaration, ExtractClassPreviewProcessor.PsiElementChange.ChangeType.ADD_BEFORE, CREATE_CLONE, sourceTypeDeclaration.getRBrace()));
                }
            }
        }
    }
    ...
    private void removeFieldFragmentsInSourceClass(Set<PsiField> fieldFragments) {
        for (PsiField field : fieldFragments) {
            field.delete();
            if (previewUsage) {
                previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(field, ExtractClassPreviewProcessor.PsiElementChange.ChangeType.REMOVE, REMOVE_EXTRACTED_FIELD + " " + field.getName()));
            }
        }
    }
    ...
    private PsiMethod getInitialSourceMethod(PsiMethod methodDeclaration) {
        if (!updatedSourceMethodToInitialCopy.containsKey(methodDeclaration)) {
            ...
            methodComparingList.add(new ExtractClassPreviewProcessor.PsiMethodComparingPair(initialSourceMethod, methodDeclaration));
        }
                    ...
                    for (int i = 0; i < statements.length; i++) {
                        ...
                        if (previewUsage && initialSourceMethod != null && rewriteAST) {
                            comparingList.add(new ExtractClassPreviewProcessor.PsiElementComparingPair(
                                    ...
                                    initialSourceMethod,
                                    UPDATED_EXTRACTED_FIELD_ASSIGNMENT));
                        }
                    }
                }
            }
