...
import com.utils.JDeodorant.math.Edge;
import com.utils.JDeodorant.math.Node;
import com.utils.JDeodorant.math.TarjanAlgorithm;
import com.utils.JDeodorant.utils.ExpressionExtractor;
import com.utils.JDeodorant.utils.MethodDeclarationUtility;
import com.utils.JDeodorant.utils.StatementExtractor;

import java.util.*;

import static com.ui.JDeodorant.ExtractClassPreviewProcessor.PsiElementChange.ChangeType.*;
...
public class ExtractClassRefactoring {
    ...
    private void updateBodyDeclarationAccessModifier(PsiClass memberType) {
        ...
        if (previewUsage) {
            previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(memberType.getModifierList(), REPLACE, CHANGED_ACCESS_TO_PUBLIC));
        }
    }
    ...
    private void setPublicModifierToSourceField(PsiField variableBinding) {
        ...
        if (previewUsage) {
            previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(variableBinding, REPLACE, CHANGED_ACCESS_TO_PUBLIC));
        }
    }
    ...
    private void createExtractedTypeFieldReferenceInSourceClass(ModifyType modifyType) {
        ...
        if (modifyType == ModifyType.SOURCE) {
            ...
            if (previewUsage) {
                previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(extractedReferenceFieldDeclaration, ADD_AFTER, CREATED_FIELD_HOLDING_REFERENCE, sourceTypeDeclaration.getLBrace()));
            }
        }
    }
    ...
    private void updateWriteObjectInSourceClass(String modifiedExtractedTypeName) {
        ...
        if (!methodFound) {
            ...
            if (previewUsage) {
                previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(writeObjectMethod, ADD_BEFORE, CREATE_WRITE_OBJECT, sourceTypeDeclaration.getRBrace()));
            }
        }
    }
    ...
    private void updateReadObjectInSourceClass(String modifiedExtractedTypeName) {
        ...
        if (!methodFound) {
            ...
            if (previewUsage) {
                previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(readObjectMethod, ADD_AFTER, CREATE_READ_OBJECT, sourceTypeDeclaration.getLBrace()));
            }
        }
    }
    ...
    private void updateCloneInSourceClass(String modifiedExtractedTypeName) {
        ...
        if (!methodFound) {
            ...
            if (cloneMethodBinding != null) {
                ...
                if (previewUsage) {
                    previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(cloneMethodDeclaration, ADD_BEFORE, CREATE_CLONE, sourceTypeDeclaration.getRBrace()));
                }
            }
        }
    }
    ...
    private void removeFieldFragmentsInSourceClass(Set<PsiField> fieldFragments) {
        for (PsiField field : fieldFragments) {
            field.delete();
            if (previewUsage) {
                previewProcessor.addToPsiElementChangesFromUpdatedClass(new ExtractClassPreviewProcessor.PsiElementChange(field, REMOVE, REMOVE_EXTRACTED_FIELD + " " + field.getName()));
            }
        }
    }
